...
# -*- coding: utf-8 -*-
"""
Kappa-FIN v4.0 — Implementação corrigida do Método Kappa (README §IV–V)
=====================================================================

Correções v4.0 em relação ao engine v3.0:

  v4-FIX-1: Ξ NAME COLLISION — Λ(t) para coupling, Ξ(t) para diversidade H1
  v4-FIX-2: SPS COMPUTATION — 6 condições conjuntas agora computadas
  v4-FIX-3: PR (Preparation Ratio) adicionado ao engine
  v4-FIX-4: _local_phi proxy corrigido (η inversamente proporcional)
  v4-REVERT-5: Forman-Ricci mantido não-ponderado (validado em 17 cenários)
  v4-FIX-6: curv_summary config conectado
  v4-FIX-7: Φ formula documentada (Oh-driven, não curvatura bruta)
  v4-FIX-8: DEF S_pre vector documentado

  Correções herdadas do v3.0 sobre o engine v4.6 original:
...
def forman_ricci(G, summary: str = "median") -> float:
    """
    Curvatura de Forman-Ricci NÃO-PONDERADA (topológica).

    FR(e) = 4 - deg(u) - deg(v) + 3 * |triângulos compartilhados|

    v4-REVERT-5: FR ponderado (exp(-d)) mudava a escala de curvatura
    de forma que destruía a calibração CALM em todos os cenários validados.
    A versão não-ponderada é a implementação validada em 17 cenários (v3).
    Documentado como decisão metodológica: FR topológico puro é mais
    estável para redes kNN com pesos de distância angular.

    v4-FIX-6: Respeita cfg.curv_summary (median/mean).
    """
    if nx is None:
        return 0.0
...
def window_state(Rw: np.ndarray, cfg: ConfigV3) -> Dict[str, float]:
    ...
    curv    = forman_ricci(G, summary=cfg.curv_summary)
...
def _local_phi(seg: pd.DataFrame, cfg: ConfigV3) -> np.ndarray:
    ...
    # v4-FIX-8: proxy Oh ∝ 1/η (Oh = η_ref/η/Λ_c, inversamente proporcional)
    # Antes: raw = v * et (multiplicativo — ERRADO)
    eta_ref_local = max(float(np.quantile(et, 0.05)), EPS)
    raw = eta_ref_local / (et + EPS)
...
def calm_ensemble_params(
    ...
    eta_refs, Lambda_cs, Oh_pres, xi_cs, scores = [], [], [], [], []
    ...
    for (cs_i, ce_i, score_i) in top:
        ...
        Lambda_i = eta_r / (eta + EPS)              # v4: renamed Xi_Oh → Lambda
        Lambda_c_i = Lambda_i[mask_i]
        Lambda_c_v = max(float(np.quantile(Lambda_c_i, 0.99)) * 1.02, EPS)
        Oh_i    = Lambda_i / Lambda_c_v
    ...
    if not eta_refs:
        ...
        Lambda0 = eta_r0 / (eta + EPS)
        Lambda_c0 = max(float(np.quantile(Lambda0[mask0], 0.99)) * 1.02, EPS)
        Oh0    = Lambda0 / Lambda_c0
        return dict(
            eta_ref  = eta_r0,
            Xi_c     = Xi_c0,
            ...
            candidates = [{
                ...
                "Lambda_c": Lambda_c0, "Oh_pre": float(np.quantile(Oh0[mask0], cfg.pre_q)),
            }],
        )

    # Agregação por mediana
    eta_ref = float(np.median(eta_refs))
    Lambda_c = float(np.median(Lambda_cs))
    ...
    candidates = [
        {"start": str(c[0].date()), "end": str(c[1].date()), "score": float(c[2]),
         ...
         "Oh_pre": float(op)}
        for c, er, xc, op in zip(top, eta_refs, Lambda_cs, Oh_pres)
    ]

    print(f"   [CALM-ensemble] N={len(eta_refs)} candidatos  "
          f"eta_ref={eta_ref:.6f}  Lambda_c={Lambda_c:.6f}  Oh_pre={Oh_pre:.4f}")
...
def compute_Oh(
    ...
    *,
    ...
    v4-FIX-2: Renamed Xi_Oh → Lambda (coupling index) to avoid
    collision with Xi (structural diversity based on h1_count).

        η_ref   = quantile(η[CALM], q=0.05)
        Λ(t)    = η_ref / η(t)                  ← coupling index
        Λ_c     = quantile(Λ[CALM], 0.99) × 1.02
        Oh(t)   = Λ(t) / Λ_c                    ← Ohio Number

    Se eta_ref e Lambda_c forem fornecidos (vindos de calm_ensemble_params),
    ...
    Lambda = eta_ref / (eta + EPS)          # v4: renamed from Xi_Oh

    if Lambda_c is None:
        Lambda_calm = Lambda[mask] if mask.sum() > 0 else Lambda
        Lambda_c = max(float(np.quantile(Lambda_calm, 0.99)) * 1.02, EPS)
    else:
        Lambda_c = max(Lambda_c, EPS)
    ...
    meta = {
        "eta_ref":     eta_ref,
        "Lambda_c":    Lambda_c,             # v4: renamed from Xi_c
...
# ═══════════════════════════════════════════════════════════════════════════════
# SPS — STRUCTURAL PRESSURIZATION SIGNATURE (v4-FIX-3)
# ═══════════════════════════════════════════════════════════════════════════════

def compute_SPS(
    Oh: np.ndarray, Phi: np.ndarray, eta: np.ndarray,
    Xi: np.ndarray, DEF: np.ndarray,
    mask: np.ndarray,
    Oh_pre: float, phi_c: float,
    eta_ref: float, xi_c: float,
) -> Dict[str, object]:
    """
    Structural Pressurization Signature — Definition 3.1 do paper.

    v4-FIX-3: Implementação completa das 6 condições conjuntas.
    Antes, classify_regime() usava apenas Oh e Phi.

    Condições (por timestep):
      (i)   Oh(t)  > Oh_pre          — pressão acima do CALM ceiling
      (ii)  Φ(t)   > Φ_c             — memória estrutural ativa
      (iii) ΔΦ(t)  > 0               — memória acumulando
      (iv)  η(t)   > η_q75_calm      — rigidez acima da mediana CALM
      (v)   Ξ(t)   < 1.0             — diversidade abaixo do CALM
      (vi)  DEF(t) < DEF_q75_calm    — baixa divergência angular

    Retorna:
      sps_mask: bool array, True onde TODAS as 6 condições são satisfeitas
      tau_SPS:  fração de timesteps com SPS ativo
      sps_onset: primeiro índice com SPS (ou None)
      sps_max_run: maior sequência consecutiva de SPS
      conditions: dict com cada condição individual (para diagnóstico)
    """
    n = len(Oh)

    # Thresholds derivados do CALM
    eta_calm = eta[mask] if mask.sum() > 0 else eta
    DEF_calm = DEF[mask] if mask.sum() > 0 else DEF
    eta_q75  = float(np.quantile(eta_calm, 0.75))
    DEF_q75  = float(np.quantile(DEF_calm, 0.75))
    ...
    # SPS = todas as 6 simultaneamente
    sps_mask = c1 & c2 & c3 & c4 & c5 & c6

    # tau_SPS = fração
    tau_SPS = float(sps_mask.mean())

    # Onset
    sps_indices = np.where(sps_mask)[0]
    sps_onset = int(sps_indices[0]) if len(sps_indices) > 0 else None
    ...
    if cur > 0:
        runs.append(cur)
    sps_max_run = max(runs) if runs else 0

    return {
        "sps_mask":     sps_mask,
        "tau_SPS":      tau_SPS,
        "sps_onset":    sps_onset,
        "sps_max_run":  sps_max_run,
        "n_sps_days":   int(sps_mask.sum()),
        "thresholds": {
            "Oh_pre":   Oh_pre,
            "phi_c":    phi_c,
            "eta_q75":  eta_q75,
        ...
        "conditions": {
            "c1_Oh_above_pre":   float(c1.mean()),
            "c2_Phi_above_c":    float(c2.mean()),
            "c3_dPhi_positive":  float(c3.mean()),
            "c4_eta_above_q75":  float(c4.mean()),
            "c5_Xi_below_1":     float(c5.mean()),
            "c6_DEF_below_q75":  float(c6.mean()),
        },
    }
...
def compute_PR(Phi: np.ndarray, event_idx: Optional[int] = None,
               lookback: int = 5) -> float:
    """
    Preparation Ratio — v4-FIX-7.

    PR = Φ_cum(t* − ε) / Φ_cum(t*)

    Measures what fraction of total structural damage was pre-existing
    before the event of reference.

    PR → 1: damage was accumulated in preceding months (endogenous).
    PR → 0: damage appeared at/after the event (exogenous).
    NaN when Φ_cum(t*) < 0.01.

    event_idx: index of the event. If None, uses argmax(Phi).
    lookback: ε in business days (default: 5).
    """
    if event_idx is None:
        event_idx = int(np.argmax(Phi))

    phi_cum = np.cumsum(Phi)
    phi_at_event = phi_cum[event_idx] if event_idx < len(phi_cum) else phi_cum[-1]

    if phi_at_event < 0.01:
        return float("nan")

    pre_idx = max(0, event_idx - lookback)
    phi_pre = phi_cum[pre_idx]

    return float(phi_pre / phi_at_event)
...
def plot_dashboard(df: pd.DataFrame, calm_end: pd.Timestamp,
    ...
    suptitle = (f"Kappa-FIN v4.0 — {title}\n"
    ...
    out_path = os.path.join(out, "kappa_v4_dashboard.png")
...
def build_report(cfg: ConfigV3, n_tickers: int, n_days: int,
    ...
    L = []
    L += ["=" * 72,
          "Kappa-FIN v4.0 — RELATÓRIO DE ANÁLISE COMPLETA",
    ...
    L += [f"  η_ref (CALM q5 de η)        = {Oh_meta['eta_ref']:.6f}",
          f"  Λ_c  (CALM q99 de Λ(η))    = {Oh_meta['Lambda_c']:.6f}",
    ...
    L += ["─" * 72, "SPS — STRUCTURAL PRESSURIZATION SIGNATURE (v4)", "─" * 72]
    L += [f"  τ_SPS (fração com 6 condições): definido em runtime",
          f"  (Use kappa_v4_state.csv coluna 'sps' para análise)",
          ""]
...
def run(cfg: ConfigV3, scenario_title: str = "") -> pd.DataFrame:
    ...
    print("\n" + "=" * 72)
    print("Kappa-FIN v4.0 — Implementação Corrigida do Método Kappa")
    ...
    Oh, Oh_meta = compute_Oh(state, mask, cfg,
                             eta_ref=ens["eta_ref"], Lambda_c=ens["Lambda_c"])
    ...
    # ── F. SPS — Structural Pressurization Signature ─────────────────────
    print("[F] Computando SPS (6 condições conjuntas)...")
    sps = compute_SPS(
        Oh, Phi, state["eta"].values, Xi, DEF, mask,
        Oh_pre, phi_c, Oh_meta["eta_ref"], xi_c,
    )
    print(f"    SPS: tau_SPS={sps['tau_SPS']:.3f}  "
          f"n_days={sps['n_sps_days']}  max_run={sps['sps_max_run']}d")
    for ck, cv in sps["conditions"].items():
        print(f"      {ck}: {cv:.1%}")

    # ── G. Viscosidade Estrutural ─────────────────────────────────────────────
    print("[G] Computando viscosidade estrutural ν_s...")
    ...
    df["Oh_pre"] = Oh_pre
    df["Xi_c"]   = xi_c
    df["sps"]    = sps["sps_mask"].astype(int)
    df.attrs["gamma"] = cfg.gamma
    df.attrs["delta"] = cfg.delta


    # ── I. PR (Preparation Ratio) ────────────────────────────────────────
    pr = compute_PR(Phi)
    viscosity["PR"] = pr
    print(f"    PR = {pr:.4f}" if np.isfinite(pr) else "    PR = NaN")
    ...
    # ── J. Salva outputs ──────────────────────────────────────────────────────
    csv_path = os.path.join(cfg.out, "kappa_v4_state.csv")
    df.to_csv(csv_path)
    print(f"[OK] State CSV: {csv_path}")

    rep_path = os.path.join(cfg.out, "kappa_v4_report.txt")
    with open(rep_path, "w", encoding="utf-8") as f:
        f.write(report)
    print(f"[OK] Relatório: {rep_path}")

    # Viscosidade CSV
    visc_path = os.path.join(cfg.out, "kappa_v4_viscosity.csv")
...
def parse_args() -> ConfigV3:
    p = argparse.ArgumentParser(
        description="Kappa-FIN v4.0 — Implementação exata do Método Kappa"
    )
