...
# -*- coding: utf-8 -*-
r"""
Kappa Method v2 -- engine_v5.py  (v5.8c -- dual-horizon + CALM_SATURATED)
===========================================================================
v5.8c changes (2026-04-01):
  FIX-12v3: CALM saturation → Theta_A=0 + CALM_SATURATED flag (defer to SIG Layer 0)
  Dual-horizon prognostic:
          - H=90 (strategic): β₀=-2.25, β_F=+1.02, β_ρ=+0.004, β_η=+0.54, β_A=+4.42
            AUC=0.733, ECE=0.089, LOUO-CV=0.645
          - H=60 (operational): β₀=-2.79, β_F=+1.13, β_ρ=+0.005, β_η=+0.55, β_A=+4.83
            AUC=0.772, ECE=0.082, LOUO-CV=0.671
          - Both pass sign stability, shuffle, random feature tests
          - Alerts driven by H=90 (conservative); H=60 provides faster operational view
...
class V2Config:
    ...
    # Calibrated H=90 coefficients (Experiment 3, v5.8c, ECE=0.089, PASS)
    beta_0: float = -2.2534; beta_F: float = 1.0169; beta_rho: float = 0.0043
    beta_eta: float = 0.5391; beta_A: float = 4.4152; csd_window: int = 40
    # Calibrated H=60 coefficients (Experiment 3, v5.8c, ECE=0.082, PASS)
    beta60_0: float = -2.7906; beta60_F: float = 1.1308; beta60_rho: float = 0.0049
    beta60_eta: float = 0.5489; beta60_A: float = 4.8312
...
def run_layer2(state_df, cfg, calm_mask=None, has_damage=False):
    ...
    if rdf is not None and len(rdf) == n:
        ...
        # FIX-12 v3: When CALM is saturated (GLOBAL_FALLBACK from compute_rqa_rolling),
        # ALWAYS zero Theta_A. The global reference z-scores are near-constant and
        # carry no discriminative temporal signal — they are a bias, not a signal.
        # These universes are "geometrically unobservable" at the RQA layer.
        # Layer 0 (SIG: SCR/lambda_1) handles them instead.
        if geo_rel in ("LOW", "GLOBAL_FALLBACK"):
            theta = theta * 0.0
            if geo_rel == "GLOBAL_FALLBACK":
                geo_rel = "CALM_SATURATED"  # informational flag
                calm_stats["_note"] = "Layer2 blind; defer to SIG Layer 0 (SCR/lam1)"
        v2g["theta_A"] = theta
...
def compute_hazard_h60(kf, rbp, en, ta, cfg):
    lo = cfg.beta60_0 + cfg.beta60_F*kf + cfg.beta60_rho*rbp + cfg.beta60_eta*en + cfg.beta60_A*ta
    return 1.0 / (1.0 + np.exp(-np.clip(lo, -20, 20)))
...
def run_layer3(v2c, v2g, sdf, cfg, cm=None, phi_star_estimated=False):
    ...
    # Dual-horizon hazard: H=90 (strategic) + H=60 (operational)
    h90 = compute_hazard_discrete(kf, rbp, en, ta, cfg)
    h60 = compute_hazard_h60(kf, rbp, en, ta, cfg)
    p30 = compute_collapse_prob(h90, 30); p60 = compute_collapse_prob(h60, 60)
    p90 = compute_collapse_prob(h90, 90); th = compute_half_life(h90)
    th60 = compute_half_life(h60)
    csd = compute_csd(v2c["C"].values, cfg.csd_window, cm)
    pcs = compute_pcs(kf, ta, csd, cfg); cn = v2c["C_norm"].values
    al = compute_alerts(cn, kf, h90, th, pcs, phi_star_estimated, cfg)
    r = pd.DataFrame(index=sdf.index)
    r["h"]=h90; r["h60"]=h60
    r["P_collapse_30"]=p30; r["P_collapse_60"]=p60; r["P_collapse_90"]=p90
    r["T_half"]=th; r["T_half_60"]=th60
...
def run_v2(state_csv, viscosity_csv, cfg=None):
    ...
    # FIX-12 v2: detect SIGNIFICANT damage history for CALM fallback gating
    # Only universes with severe structural damage (min C_norm < 0.50) qualify
    # for GLOBAL_FALLBACK. Marginal dips (C_norm briefly below 0.90) → Theta_A=0.
    c_norm_vals = v2c["C_norm"].values
    has_damage = bool(np.min(c_norm_vals) < 0.50)
    ...
    if calm_stats.get("_note"):
        cs_str += f" [{calm_stats['_note']}]"
    ...
    print("[v2] Layer 3 -- Prognostic Module (dual-horizon)...")
    v2p = run_layer3(v2c, v2g, sdf, cfg, cm, phi_star_estimated)
    hn = float(v2p["h"].iloc[-1]); hn60 = float(v2p["h60"].iloc[-1])
    pn = float(v2p["P_collapse_30"].iloc[-1]); p60n = float(v2p["P_collapse_60"].iloc[-1])
    thn = float(v2p["T_half"].iloc[-1]); an = v2p["alert_level"].iloc[-1]
    print(f"     H90: h={hn:.6f}  P30={pn:.4f}  P90={float(v2p['P_collapse_90'].iloc[-1]):.4f}  T1/2={thn:.1f}")
    print(f"     H60: h={hn60:.6f}  P60={p60n:.4f}  T1/2={float(v2p['T_half_60'].iloc[-1]):.1f}")
    print(f"     [{an}]")
    ...
    s = {
        ...
        "h_now": hn, "h60_now": hn60,
        "P_collapse_30d": pn,
        "P_collapse_60d": p60n,
        "P_collapse_90d": float(v2p["P_collapse_90"].iloc[-1]),
        "T_half_now": thn, "T_half_60_now": float(v2p["T_half_60"].iloc[-1]),
