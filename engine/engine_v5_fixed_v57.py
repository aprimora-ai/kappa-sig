...
# -*- coding: utf-8 -*-
r"""
Kappa Method v2 -- engine_v5.py  (v5.8 -- FIX-12 CALM layer-conditional normalization)
===========================================================================
v5.8 changes (2026-04-01):
  FIX-12: CALM layer-conditional geometric normalization
          - When CALM baseline is geometrically saturated (sigma < GEO_SIGMA_MIN),
            use global corpus RQA reference stats instead of zeroing Theta_A
          - Principle: "CALM remains foundational for structural baselining;
            geometric observability is not guaranteed by structural calmness"
          - Affects: commodities, financials (ALREADY_CRYSTALLIZED in v2.5)
          - New geo_reliability level: GLOBAL_FALLBACK (vs HIGH/LOW)
          - Evidence: Experiment 4e showed +5.3% AUC when CALM attenuation removed
...
def run_layer2(state_df, cfg, calm_mask=None):
    ...
    if rdf is not None and len(rdf) == n:
        ...
        # FIX-12: Only zero Theta_A when truly no geometric data (LOW).
        # GLOBAL_FALLBACK means CALM was saturated but we used corpus reference.
        # HIGH means normal CALM z-score worked fine.
        if geo_rel == "LOW": theta = theta * 0.0
        # GLOBAL_FALLBACK: theta is computed from global reference — keep it
        v2g["theta_A"] = theta
